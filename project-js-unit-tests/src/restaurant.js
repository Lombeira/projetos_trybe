...
const createMenu = (obj) => {
  const objMenu = obj !== undefined ? obj : { food: {}, drink: {} };
  const items = { ...objMenu.food, ...objMenu.drink };
  const menu = {
    products: items,
    fetchMenu: () => objMenu,
    consumption: [],
    order: (value) => menu.consumption.push(value),
    pay() {
      let value = 0;
      for (let index = 0; index < menu.consumption.length; index += 1) {
        value += menu.products[menu.consumption[index]];
      }
      return value;
    },
  };
