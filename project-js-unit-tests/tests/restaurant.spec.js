...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    assert.ok(Object.keys(objetoRetornado).length > 1);

    assert.deepStrictEqual(
      Object.keys(objetoRetornado),
      Object.keys({
        products: {},
        fetchMenu: () => {},
        consumption: [],
        order: (value) => objetoRetornado.consumption.push(value),
        pay() {
          let value = 0;
          for (
            let index = 0;
            index < objetoRetornado.consumption.length;
            index += 1
          ) {
            value +=
              objetoRetornado.products[objetoRetornado.consumption[index]];
          }
          return value;
        },
      })
    ...
    objetoRetornado.order('agua');
    objetoRetornado.order('coxinha');
    assert.strictEqual(objetoRetornado.pay(), 3.9 + 3.9 + 3.9);
    objetoRetornado.order('sopa');
    assert.strictEqual(objetoRetornado.pay(), 3.9 + 3.9 + 3.9 + 9.9);
  });
});
